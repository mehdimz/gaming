...
router.get('/platform/:name', function(req, res, next) {
  async.autoInject({
    ...
    items: function(platform, cb) {
      return Cat.find({ hidden: { $ne: true }, _platforms: platform._id }).sort('-_id').exec(cb);
    },
    populars: function(cb) {
      return Clip.find().sort({ viewCount: -1 }).limit(4).exec(cb);
    },
...
router.get('/genre/:name', function(req, res, next) {
  async.autoInject({
    ...
    items: function(genre, cb) {
      return Cat.find({ hidden: { $ne: true }, _genres: genre._id }).sort('-_id').exec(cb);
    },
    populars: function(cb) {
      return Clip.find().sort({ viewCount: -1 }).limit(4).exec(cb);
    },
...
router.get('/:title/:id', function(req, res, next) {
  async.parallel({
    ...
    news: (cb) => New.find({ _category: req.params.id }).limit(10).exec(cb)
  }, function(err, data) {
    if (err) return next(err);
