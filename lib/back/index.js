...
router.get('/', function(req, res, next) {
  async.parallel({
    ...
    toBeReleasedGames: function(cb) {
      ...
      }).populate('_platforms').limit(9).exec(cb);
    },
    trailers: function(cb) {
      return Clip.find({isTrailer: true}).populate({
        path: '_category',
        populate: {
          path: '_platforms'
        }
      }).sort('-_id').limit(8).exec(cb);
    },
    news: function(cb) {
      return New.find().sort('-_id').limit(7).exec(cb);
    },
    bannerNews: function(cb) {
      return New.find({inBannerSmall: true}).sort('-_id').limit(7).exec(cb);
    },
