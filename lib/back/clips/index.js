...
router.post('/', function(req, res, next) {
  ...
  form.parse(req, function(err, fields, files) {
    fields.isTrailer = !!fields.isTrailer;
    fields.inBanner = !!fields.inBanner;
    if (fields._category == '') {
...
router.post('/:id', function(req, res, next) {
  ...
  form.parse(req, function(err, fields, files) {
    ...
    if (fields._category == '') {
      newFields['$unset'] = { _category: 1 };
      delete fields._category;
    }

    newFields['$set'] = fields;

    Clip.findByIdAndUpdate(req.params.id, newFields, function(err, clip) {
      if (err) return next(err);
      if (files.image.size) {
        clip.saveImage(files.image, function(err) {
          if (err) return next(err);
