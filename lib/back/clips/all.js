...
router.get('/platform/:name', function(req, res, next) {
  async.autoInject({
    ...
    cats: function(platform, cb) {
      return Cat.find({ '_platforms': platform._id, hidden: { $ne: true } }).exec(cb);
    },
...
router.get('/genre/:name', function(req, res, next) {
  async.autoInject({
    ...
    cats: function(genre, cb) {
      return Cat.find({ '_genres': genre._id, hidden: { $ne: true } }).exec(cb);
    },
    items: function(cats, cb) {
      return Clip.find({ '_category': { $in: cats } }).sort('-_id').exec(cb);
    },
  ...
  async.parallel({
    item: function(cb) {
      return Clip.findOne({ _id: req.params.id })
        ...
        .populate({ path: '_category._platforms', select: 'name' })
        .exec(function(err, item) {
          async.series([
            function(callback) {
              return Genre.populate(item, { path: '_category._genres', select: 'name' }, callback);
            },
            function(callback) {
              return Platform.populate(item, { path: '_category._platforms', select: 'name' }, callback);
            }
          ], function(err) {
            if (err) return next(err);
            return cb(null, item);
