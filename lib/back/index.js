...
router.get('/', function(req, res, next) {
  async.parallel({
    ...
    todayClips: function(cb) {
      ...
      }).limit(8).sort('-_id').exec(cb);
    },
    bannerGames: function(cb) {
      return Cat.find({ inBanner: true }).limit(3).exec(cb);
    },
    cats: function(cb) {
      return Cat.find({ inCarousel: true }).limit(8).exec(cb);
    },
    naghds: function(cb) {
      return Cat.find({
        inReview: true,
    ...
    news: function(cb) {
      return New.find().sort('-_id').limit(3).exec(cb);
    }
  }, function(err, data) {
