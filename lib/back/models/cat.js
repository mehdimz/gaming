...
CatShema.methods.saveImage = function(file, cb) {
  ...
  async.eachSeries([{ w: 1000, h: 750 }, { w: 500, h: 375 }, { w: 250, h: 187 }], function iteratee(item, callback) {
    lwip.open(file.path, 'JPEG', function(err, img) {
      ...
      img.batch().resize(item.w, item.h).writeFile(__dirname + '/../../storage/images/cats/' + item.w + '/' + _this._id + '.jpg', 'jpg', { quality: 60 }, callback);
    });
  }, cb);
};
...
CatShema.methods.saveSlider = function(file, cb) {
  ...
  async.eachSeries([{ w: 1600, h: 645 }], function iteratee(item, callback) {
    lwip.open(file.path, 'JPEG', function(err, img) {
      if (err) return callback(err);
      img.batch().resize(item.w, item.h).writeFile(__dirname + '/../../storage/images/sliders/' + item.w + '/' + _this._id + '.jpg', 'jpg', { quality: 60 }, callback);
    });
  }, cb);
};
...
CatShema.methods.saveAdditionalImage = function(file, cb) {
  ...
  async.eachSeries([{ w: 'big' }, { w: 250, h: 187 }], function iteratee(item, callback) {
    lwip.open(file.path, 'JPEG', function(err, img) {
      if (err) return callback(err);
      if (typeof item.w == 'string') {
        img.writeFile(__dirname + '/../../storage/images/additional/' + item.w + '/' + imageName + '.jpg', 'jpg', { quality: 60 }, callback);
      } else {
        img.batch().resize(item.w, item.h).writeFile(__dirname + '/../../storage/images/additional/' + item.w + '/' + imageName + '.jpg', 'jpg', { quality: 60 }, callback);
      }
    });
  }, function(err) {
    cb(err, imageName);
